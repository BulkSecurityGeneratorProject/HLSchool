        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.postService.find(id).subscribe((post) => {
                    post.createDate = this.datePipe
                        .transform(post.createDate, 'yyyy-MM-ddTHH:mm:ss');
                    post.lastModifier = this.datePipe
                        .transform(post.lastModifier, 'yyyy-MM-ddTHH:mm:ss');
